...
!function (e, t) {
  ...
  function ab(e, t, r, n) {
    if (e.parentNode !== t) {
      ...
      a.position = "absolute" === r.position ? "absolute" : "relative", "inline" === r.display && (a.display = "inline-block"), s[y] = s[m] = "auto", a.overflow = "visible", a.boxSizing = "border-box", a[Ke] = ua(e, tt) + et, a[je] = ua(e, rt) + et, a[Ze] = s[$e] = s.top = s[x] = "0", ct(n), s[Ke] = s.maxWidth = r[Ke], s[je] = s.maxHeight = r[je], s[Ze] = r[Ze], e.parentNode.insertBefore(t, e), t.appendChild(e)
    }
  }
  ...
  var Z = (ScrollTrigger.prototype.init = function init(m, y) {
    if (this.progress = 0, this.vars && this.kill(1), Je) {
      ...
      Te.media = We, ue *= 45, at.push(Te), Te.scroller = he, Te.scroll = M(he, te), C = Te.scroll(), Te.vars = m, y = y || m.animation, "refreshPriority" in m && (Ne = 1), f.tweenScroll = f.tweenScroll || {
        top: jb(he, rt),
        left: jb(he, tt)
      }, Te.tweenTo = d = f.tweenScroll[te.p], y && (y.vars.lazy = !1, y._initted || !1 !== y.vars.immediateRender && !1 !== m.immediateRender && y.render(0, !0, !0), Te.animation = y.pause(), y.scrollTrigger = Te, (o = U(a) && a) && (Y = Me.to(y, {
        ease: "power3",
        duration: o,
        onComplete: function onComplete() {
          return s && s(Te)
        }
      })), b = 0, i = i || y.vars.id), pe && (V(pe) || (pe = { snapTo: pe }), Me.set(ve ? [
        Oe,
        _e
      ...
      }(y) : Me.utils.snap(pe.snapTo), c = pe.duration || {
        min: .1,
        max: 2
        ...
        if (Math.abs(Te.getVelocity()) < 10 && !Ae) {
          ...
          if (i <= _ && P <= i && a !== i) {
            ...
            d(a, {
              ...
              data: Math.abs(a - i),
              onComplete: function onComplete() {
                b = X = y && !ge ? y.totalProgress() : Te.progress, u && u(Te)
              }
            }, i, r * A, a - i - r * A)
          }
        ...
        targets: ae,
        className: ne
      }), se && (!1 === le || le === $e || (le = !(!le && "flex" === qa(se.parentNode).display) && Ze), Te.pin = se, !1 !== m.force3D && Me.set(se, { force3D: !0 }), (n = Me.core.getCache(se)).spacer ? z = n.pinState : (n.spacer = B = Pe.createElement("div"), B.setAttribute("class", "pin-spacer" + (i ? " pin-spacer-" + i : "")), n.pinState = z = db(se)), Te.spacer = B = n.spacer, r = qa(se), h = r[le + te.os2], F = Me.getProperty(se), g = Me.quickSetter(se, te.a, et), ab(se, B, r), W = db(se)), p && (e = V(p) ? sa(p, ot) : ot, I = Da("scroller-start", i, he, te, e, 0), R = Da("scroller-end", i, he, te, e, 0, I), t = I["offset" + te.op.d2], O = Da("start", i, he, te, e, t), E = Da("end", i, he, te, e, t), xe || (function _makePositionable(e) {
        ...
        }(se, B, z) : de && Te.isActive || ab(se, B, qa(se), j)), w = t)
        ...
        if (!Re && Te.enabled) if (se && e && De) xa(ScrollTrigger, "scrollEnd", Qa); else {
          ...
          for (var t, r, n, o, i, a, s, l, c = Se(), u = Ce(), f = Q(he, te), p = 0, d = 0, g = m.end, h = m.endTrigger || ae, v = m.start || (0 === m.start ? 0 : se || !ae ? "0 0" : "0 100%"), x = ae && Math.max(0, at.indexOf(Te)) || 0, b = x; b--;) !(s = at[b].pin) || s !== ae && s !== se || at[b].revert();
          for (P = gb(v, ae, c, te, Te.scroll(), O, I, Te, u, ye, xe, f) || (se ? -.001 : 0), T(g) && (g = g(Te)), S(g) && !g.indexOf("+=") && (~g.indexOf(" ") ? g = (S(v) ? v.split(" ")[0] : "") + g : (p = Ca(g.substr(2), c), g = S(v) ? v : P + p, h = ae)), _ = Math.max(P, gb(g || (h ? "100% 0" : f), h, c, te, Te.scroll() + p, E, R, Te, u, ye, xe, f)) || -.001, A = _ - P || (P -= .01) && .001, p = 0, b = x; b--;) (s = (a = at[b]).pin) && a.start - a._pinPush < P && (t = a.end - a.start, s === ae && (p += t), s === se && (d += t));
          if (P += p, _ += p, Te._pinPush = d, O && p && ((t = {})[te.a] = "+=" + p, Me.set([
            O,
            E
          ], t)), se) t = qa(se), o = te === rt, n = Te.scroll(), D = parseFloat(F(te.a)) + d, !f && 1 < _ && ((ve ? Oe : he).style["overflow-" + te.a] = "scroll"), ab(se, B, t), W = db(se), r = nt(se, !0), l = xe && M(he, o ? tt : rt)(), le && ((j = [
            le + te.os2,
            A + d + et
          ]).t = B, (b = le === Ze ? ua(se, te) + A + d : 0) && j.push(te.d, b + et), ct(j), xe && Te.scroll(G)), xe && ((i = {
            ...
            return o.t = e.t, o
          }(z, i, de)), y ? (y.progress(1, !0), J = F(te.a) - D + A + d, A !== J && q.splice(q.length - 2, 2), y.progress(0, !0)) : J = A; else if (ae && Te.scroll()) for (r = ae.parentNode; r && r !== Oe;) r._pinOffset && (P -= r._pinOffset, _ -= r._pinOffset), r = r.parentNode;
          for (b = 0; b < x; b++) !(a = at[b].pin) || a !== ae && a !== se || at[b].revert(!1);
        ...
        if (t && (k = C, C = s, pe && (X = b, b = y && !ge ? y.totalProgress() : c)), ue && !c && se && !Re && !Be && De && P < s + (s - k) / (He() - Ie) * ue && (c = 1e-4), c !== u && Te.enabled) {
          ...
          !pe || d.tween || Re || Be || Z.restart(!0), ne && (a || fe && c && (c < 1 || !qe)) && Ee(ne.targets).forEach(function (e) {
            return e.classList[r || fe ? "add" : "remove"](ne.className)
          }), !re || ge || e || re(Te), i && !Re ? (n = c && !u ? 0 : 1 === c ? 1 : 1 === u ? 2 : 3, ge && (o = !a && "none" !== me[n + 1] && me[n + 1] || me[n], y && ("complete" === o || "reset" === o || o in y) && ("complete" === o ? y.pause().totalProgress(1) : "reset" === o ? y.restart(!0).pause() : y[o]()), re && re(Te)), !a && qe || (oe && a && oe(Te), be[n] && be[n](Te), fe && (1 === c ? Te.kill(!1, 1) : be[n] = 0), a || be[n = 1 === c ? 1 : 3] && be[n](Te))) : ge && re && !Re && re(Te)
        }
        ...
        if (Te.enabled && (!1 !== e && Te.revert(), Te.enabled = Te.isActive = !1, t || Y && Y.pause(), G = 0, n && (n.uncache = 1), we && ya(ScrollTrigger, "refreshInit", we), Z && (Z.pause(), d.tween && d.tween.kill() && (d.tween = 0)), !ve)) {
    ...
    if (!i && (Me = e || J(), I() && window.document && (ke = window, Pe = document, _e = Pe.documentElement, Oe = Pe.body), Me && (Ee = Me.utils.toArray, Le = Me.utils.clamp, Me.core.globals("ScrollTrigger", ScrollTrigger), Oe))) {
  ...
  function ScrollTrigger(e, t) {
    i || ScrollTrigger.register(Me) || console.warn("Please gsap.registerPlugin(ScrollTrigger)"), this.init(e, t)
  }

  Z.version = "3.5.2", Z.saveStyles = function (e) {
